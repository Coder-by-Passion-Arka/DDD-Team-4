...
userSchema.pre("save", async function (next) {
  ...
  // If user has registered from Registration Page then hash the password
  else {
    try {
      const saltRounds = parseInt(process.env.BCRYPT_ROUNDS) || 12;
      this.userPassword = await bcrypt.hash(this.userPassword, saltRounds);
      console.log(`The password for ${this.userName} has been hashed`);
      next();
    } catch (error) {
      next(error);
    }
...
userSchema.virtual('isProfileComplete').get(function() {
  return !!(
    ...
    this.countryCode && 
    this.userLocation?.homeAddress && 
    this.userLocation?.currentAddress
  );
});
...
userSchema.statics.findByCredentials = async function (email, password) {
  const user = await this.findOne({
    userEmail: email.toLowerCase().trim(), // Add normalization here
  }).select("+userPassword");
