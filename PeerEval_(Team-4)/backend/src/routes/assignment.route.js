...
router.route("/course/:courseId").get((request, response, next) => {
  request.query.courseId = request.params.courseId;
  next();
}, getAssignments);
// 11. Get user's assignments with submission status (students)
router.route("/user/my-assignments").get((request, response, next) => {
