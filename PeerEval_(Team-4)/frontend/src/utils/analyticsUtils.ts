    ...
    case "weekly":
      ...
      return weeklyBase.map((item) => ({
        ...
        submissions: Math.max(
          1,
          Math.floor(
            item.baseSubmissions * (1 + (Math.random() - 0.5) * baseVariation)
          )
        ),
        evaluations: Math.max(
          1,
          Math.floor(
            item.baseEvaluations * (1 + (Math.random() - 0.5) * baseVariation)
          )
        ),
      }));
...
export const calculateAnalytics = (
  data: ProgressData[],
  activeView: "submissions" | "evaluations"
): AnalyticsMetrics => {
  const values = data.map((item) =>
    activeView === "submissions" ? item.submissions : item.evaluations
  );
...
export const generateInsights = (
  ...
  if (analytics.growthRate > 10) {
    insights.push(
      ...
      }% growth this ${timeFrame.slice(0, -2)}`
    );
  } else if (analytics.growthRate < -10) {
    insights.push(
      `📉 Consider reviewing your ${timeFrame.slice(0, -2)} strategy`
    );
  }
  ...
  if (analytics.averageDaily > 5) {
    insights.push(
      `🔥 Consistent daily activity with ${analytics.averageDaily} avg submissions`
    );
  }
