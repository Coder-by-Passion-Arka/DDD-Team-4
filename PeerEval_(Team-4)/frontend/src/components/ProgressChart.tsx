...
const ProgressChart: React.FC<ProgressChartProps> = () => {
  const [activeView, setActiveView] = useState<"submissions" | "evaluations">(
    "submissions"
  );
  ...
  const maxValue =
    activeView === "submissions" ? maxSubmissions : maxEvaluations;

  const totalSubmissions = aggregatedData.reduce(
    (sum, d) => sum + d.submissions,
    0
  );
  const totalEvaluations = aggregatedData.reduce(
    (sum, d) => sum + d.evaluations,
    0
  );
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 shadow-lg dark:shadow-gray-900/30 border border-gray-100 dark:border-gray-700">
      ...
      <div className="flex items-center justify-center mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-100 dark:border-gray-700">
        <div className="flex items-center space-x-2">
          ...
          ></div>
          <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
            {activeView === "submissions" ? "Submissions" : "Evaluations"} -{" "}
            {getChartTitle().toLowerCase()}
          </span>
        </div>
      </div>
    </div>
...
export default ProgressChart;
>>>>>>> 7516fee1fac1d632c662efaa47b95de8a3316a8b
