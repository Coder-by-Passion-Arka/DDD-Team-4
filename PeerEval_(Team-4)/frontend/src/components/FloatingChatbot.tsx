...
const FloatingChatbot: React.FC = () => {
  ...
  return (
    <>
        ...
        <div className={`fixed bottom-16 sm:bottom-20 right-2 sm:right-6 w-72 sm:w-80 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 z-50 transition-all duration-300 ${isMinimized ? 'h-16' : 'h-80 sm:h-96'}`}>
          <div className="flex items-center justify-between p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-t-2xl">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <h3 className="font-semibold text-white text-xs sm:text-sm">PeerBot</h3>
                <p className="text-xs text-white/80 hidden sm:block">Ask about Peer Evaluation</p>
              </div>
            </div>
            <div className="flex items-center space-x-1">
              <button onClick={toggleMinimize} className="p-1 sm:p-1.5 hover:bg-white/20 rounded-lg transition-colors duration-200">
                {isMinimized ? <Maximize2 className="w-3 h-3 sm:w-4 sm:h-4 text-white" /> : <Minimize2 className="w-3 h-3 sm:w-4 sm:h-4 text-white" />}
              </button>
              <button onClick={toggleChat} className="p-1 sm:p-1.5 hover:bg-white/20 rounded-lg transition-colors duration-200">
                ...
                {messages.map((message) => (
                  <div key={message.id} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className="max-w-[85%] sm:max-w-[80%]">
                      <div className={`px-2 sm:px-3 py-1.5 sm:py-2 rounded-2xl text-xs sm:text-sm ${message.type === 'user' ? 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-br-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white rounded-bl-md'}`}>
      ...
      <button
        onClick={toggleChat}
        className={`fixed bottom-4 sm:bottom-6 right-4 sm:right-6 w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 z-50 flex items-center justify-center group ${isOpen ? 'scale-0' : 'scale-100'}`}
      >
        ...
        <div className="absolute -top-1 -right-1 w-3 h-3 sm:w-4 sm:h-4 bg-red-500 rounded-full flex items-center justify-center">
          <span className="text-xs font-bold text-white hidden sm:inline">!</span>
        </div>
