...
const GameModal: React.FC<GameModalProps> = ({ isOpen, onClose }) => {
  ...
  const renderGame = () => {
    switch (currentGame) {
      ...
      case 'memory':
        return <MemoryMatch onBack={handleBackToMenu} onComplete={handleGameComplete} />;
      case 'minesweeper':
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">
                {currentGame === 'menu' ? 'Game Center' : games.find(g => g.id === currentGame)?.name}
              </h2>
              <p className="text-sm text-white/80">
                {currentGame === 'menu' ? 'Choose your adventure!' : 'Have fun and earn points!'}
              </p>
            </div>
          </div>
          <button
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-xl p-4 text-center border border-blue-200 dark:border-blue-800">
                  ...
                  <p className="text-2xl font-bold text-blue-800 dark:text-blue-200">{gameStats.gamesPlayed}</p>
                  <p className="text-sm text-blue-600 dark:text-blue-400">Games Played</p>
                </div>
                <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-900/20 dark:to-emerald-800/20 rounded-xl p-4 text-center border border-emerald-200 dark:border-emerald-800">
                  ...
                  <p className="text-2xl font-bold text-emerald-800 dark:text-emerald-200">{gameStats.gamesWon}</p>
                  <p className="text-sm text-emerald-600 dark:text-emerald-400">Games Won</p>
                </div>
                <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-xl p-4 text-center border border-purple-200 dark:border-purple-800">
                  ...
                  <p className="text-2xl font-bold text-purple-800 dark:text-purple-200">{gameStats.totalScore}</p>
                ...
                <div className="bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/20 dark:to-amber-800/20 rounded-xl p-4 text-center border border-amber-200 dark:border-amber-800">
                  ...
                  <p className="text-2xl font-bold text-amber-800 dark:text-amber-200">{gameStats.bestStreak}</p>
                  <p className="text-sm text-amber-600 dark:text-amber-400">Best Streak</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {games.map((game) => (
                  ...
                  >
                    <div className={`w-16 h-16 rounded-xl bg-gradient-to-br ${game.color} flex items-center justify-center text-2xl mb-4 group-hover:scale-110 transition-transform duration-300`}>
                      {game.icon}
                    </div>
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span className={`px-2 py-1 rounded-full ${
                        game.difficulty === 'Easy' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' :
                        game.difficulty === 'Medium' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200' :
              ...
              <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <p className="text-sm font-medium text-blue-800 dark:text-blue-200">Gaming Tips</p>
                    <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                      Playing games helps improve cognitive skills, memory, and problem-solving abilities. 
                      Take breaks between study sessions and enjoy these brain-training games!
                    </p>
                  </div>
                </div>
              </div>
