...
interface LeaderboardEntry {
  id: string;
  name: string;
  avatar: string; // Assuming this will be derived from userProfileImage or a default
  score: number;
...
const LeaderBoardService = {
  async getLeaderboardData(): Promise<LeaderboardEntry[]> {
    try {
      const response = await fetch("http://localhost:8024/api/v1/leaderboard");
      if (!response.ok) throw new Error("Failed to fetch leaderboard data");
      const data: { success: boolean; data: BackendLeaderboardEntry[] } =
        await response.json();

      if (!data.success) throw new Error("Backend API reported an error");

      // Map backend data to LeaderboardEntry interface
      const mappedData: LeaderboardEntry[] = data.data
        .map((entry) => ({
          id: entry._id,
          name: entry.displayName || entry.userName,
          avatar: entry.userProfileImage
            ? entry.userProfileImage.charAt(0).toUpperCase()
            : entry.displayName
            ? entry.displayName.charAt(0).toUpperCase()
            : entry.userName.charAt(0).toUpperCase(), // Use first letter of name as avatar for now
          score: entry.statistics.averageScore,
          rank: 0, // Will be set after sorting
          change: 0, // Placeholder
          evaluationsCompleted: 0, // Placeholder
